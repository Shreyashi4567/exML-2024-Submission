...
def handlemove(obj,move):
    ...
    if player_move_docked and ghost_move_docked:
        ...
        socketio.emit('update_positions', {
            ...
            'ghost_points': [ghost.points for ghost in ghosts],
...
def update_positions(data):
    # This is the event handler, but the positions, points, and times are emitted from the server side, as shown in the previous code.
    print(f"Updated positions: {data}")
...
socketio.emit('board', [board.get_board(), player.points])
socketio.emit('game-over', {'player_time': player_time, 'ghost_time': ghost_time})

print("Emitting board: ", board.get_board())
socketio.emit('board', [board.get_board(), player.points])
...
def initialize():
    global player, ghosts, board, player_move_docked, ghost_move_docked, player_connected, ghost_connected, player_name, ghost_name, player_timestamp, ghost_timestamp, player_time, ghost_time, moves
...
def move_player(player, move):
    ...
    for i in range(4):
        if move[i] == 1:
            return player.move(board, directions[i])

def handle_move(obj, move):
    global player_move_docked, ghost_move_docked, player_timestamp, ghost_timestamp, player_time, ghost_time, moves
    if obj == "player" and not player_move_docked:
        player_time += time.time() - player_timestamp
        player_move_docked = True
        socketio.emit('playerdocked')
        print("player move received")

    if obj == "ghost" and not ghost_move_docked:
        ghost_time += time.time() - ghost_timestamp
        ghost_move_docked = True
        socketio.emit('ghostdocked')
        print("ghost move received")
    ...
    if player_move_docked and ghost_move_docked:
        ...
        player_move_docked = False
        ghost_move_docked = False
        player_status = move_player(player, move['player_move'])
        move_ghosts(ghosts, move['ghost_move'])
        socketio.emit('undock')
        socketio.emit('board', [board.get_board(), player.points])
        socketio.emit('update_positions', {
            'player_position': player.position,
        ...
        # End game logic based on player status or board state
        if player_status == "death" or any(ghost.status == "death" for ghost in ghosts):
            player_time = round(player_time, 3)
            ghost_time = round(ghost_time, 3)
            socketio.emit('game-over', {'winner': 'ghost', 'timestamps': [player_time, ghost_time], 'moves': moves})
            socketio.emit('reset', to=player_id)
...
def scoring(points, timestamps, board_clear):
    player_time, ghost_time = timestamps
    total = points + (100 if board_clear else 0) + (player_time - ghost_time)
    return round(total, 3)

@app.route('/token-expire', methods=['POST'])
def token_expire_route():
    socketio.emit('token-refresh')
    print("token expired")
    return flask.jsonify({"status": "success"})
...
def ghost_move():
    ...
    if not ghost_connected:
        return flask.jsonify({"error": "Ghost not connected"}), 400
    handle_move("ghost", move)
    return flask.jsonify({"status": "success"})

@socketio.on('connect')
def handle_connect():
    global player_connected, ghost_connected, player_name, ghost_name, player_timestamp, ghost_timestamp, participant_id, participant_token
    origin = request.headers.get('Referer')

    if origin:
        socketio.emit('board', [board.get_board(), player.points])
        socketio.emit("position", {"position": "observer", "board": board.get_board()})
